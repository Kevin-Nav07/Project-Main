...
uniform vec3 uLightDir;      // Incoming light direction (normalized)
uniform vec3 uLightColor;    // sun color/brightness
uniform vec3 uAmbientColor;  // Ambient light

out vec4 fragColor;

// Simple pseudo-random function based on a 2D coordinate
float rand(vec2 co) {
  return fract(sin(dot(co, vec2(12.9898, 78.233))) * 43758.5453);
}

// Smooth interpolation noise function
float noise(vec2 st) {
  vec2 i = floor(st);
  vec2 f = fract(st);
  float a = rand(i);
...
void main() {
  ...
  // Compute procedural noise for color blending
  ...
  // Compute lighting, diffuse Lambert
